...
namespace DB
{
    public partial class Home : Form
    {
        ...
        private void cậpNhậtDanhSáchSoSánhToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(Home.sqlConnect))
            {
                ...
                foreach (string s in listDayDu2TrangThai)
                {
                        ...
                        if (!tt6.Equals(tt5))
                        {
                            ...
                            foreach (Diff diff in listDiff)
                            {
                                ...
                                SqlCommand cmd = new SqlCommand("use HoTich; if(select count(*) from Diff " +
                                    ...
                                    "' and ktbm2 = N'" + Utils.QuotationMarks(diff.ktbm2) + "' and kt = N'" + Utils.QuotationMarks(diff.kt) + "') = 0 " +
                                    ...
                                    "N'" + Utils.QuotationMarks(diff.ktbm2) + "',N'" + Utils.QuotationMarks(diff.kt) + "')", conn);
                                cmd.ExecuteNonQuery();
                                conn.Close();
                            }
                        }
                        ...
                        tt5.GetType().GetProperty("ID1").GetValue(tt5, null) == null))
                    {
                        ...
                        tt5.GetType().GetProperty("ID1").GetValue(tt5, null) != null))
                    {
                        if(!tt7.Equals(tt6) || !tt7.Equals(tt5) || !tt5.Equals(tt6))
                        {
                            List<Diff> listDiff = new List<Diff>();
